...
def createFuzzyControllerObstacle():
    ...
    ant6['obsProche'] = fuzz.trapmf(ant6.universe, [0, 0, 30, 50])
    ant6['obsLoin'] = fuzz.trapmf(ant6.universe, [30, 50, 80, 80])

    # ant7['obsProche'] = fuzz.trapmf(ant7.universe, [0, 0, 0.4, 0.7])
    # ant7['obsLoin'] = fuzz.trapmf(ant7.universe, [0.6, 1, 1.5, 1.5])

    ant8['murProche'] = fuzz.trapmf(ant8.universe, [0, 0, 30, 50])
    ant8['murLoin'] = fuzz.trapmf(ant8.universe, [30, 50, 80, 80])
    ...
    # 1 mur, 1 obstacle
    rules.append(ctrl.Rule(antecedent=((ant1['obsGauche'] & ant6['obsProche']) & (ant3['murGauche'] & ant8['murProche'])), consequent=cons1['tourneDroit'] % 1.5))
    rules.append(ctrl.Rule(antecedent=((ant1['obsDroit'] & ant6['obsProche']) & (ant3['murDroit'] & ant8['murProche'])), consequent=cons1['tourneGauche'] % 1.5))
...
class LogiqueFlou:
    ...
    def run(self, last_direction, next_direction, player, perception):
        ...
        self.associer_input_flou(1, distances, 'dist_obstacle', 1.5)
        ...
        self.associer_input_flou(1, distances, 'dist_mur', 1.5)
